...
I downloaded every MCP server I could find — 2,500 from npm and 841 from the official MCP registry — and ran static analysis against all of them. The results are worse than I expected.
...
That's not a typo. Nearly **28% of MCP servers** have at least one security issue worth flagging.
...
| `firebase-tools` | 340 | multiple | eval + browser profiles + env exfil + crypto wallets + obfuscation |
...
[SafeSkill](https://github.com/gabchess/safeskill) is open source. Install it as an MCP server and ask your agent "Is my MCP setup safe?" — or run the CLI directly:
